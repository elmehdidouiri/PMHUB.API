...
namespace PMHUB.API.Controllers
{
    ...
    public class ProjectFilesController : ControllerBase
    {
        ...
        public ProjectFilesController(
            IProjectFileService service,
            ILogger<ProjectFilesController> logger)
        {
            _service = service;
            _logger = logger;
        }
        ...
        [HttpGet]
        public async Task<ActionResult<ApiResponse<IEnumerable<ProjectFileDto>>>> GetAll(
            Guid projectId)
        {
        ...
        public async Task<ActionResult<ApiResponse<ProjectFileDto>>> Upload(
            Guid projectId, [FromForm] UploadProjectFileDto dto)
        {
        ...
        public async Task<ActionResult<ApiResponse<ProjectFileDto>>> Update(
            Guid projectId, Guid id, [FromBody] UpdateProjectFileDto dto)
        {
        ...
        [HttpPost("bulk")]
        [Consumes("multipart/form-data")]
        public async Task<ActionResult<ApiResponse<IEnumerable<ProjectFileDto>>>> UploadBulk(
            Guid projectId, [FromForm] BulkUploadDto dto)
        {
            if (dto.Files.Count == 0)
            {
                 dto.Files = Request.Form.Files.ToList();
                dto.FileTypes = Request.Form["FileType"]
                    .Select(x => int.Parse(x!)).ToList();
                dto.Descriptions = Request.Form["Description"]
            ...
            return Ok(ApiResponse<IEnumerable<ProjectFileDto>>.Ok(
                result, "Fichiers uploadés avec succès."));
        }
    }
}