...
namespace PMHUB.Application.Services.Implementation
{
    public class HourEntryService : IHourEntryService
    {
        ...
        {
            _logger.LogInformation("Début CreateAsync — User: {UserId}, Project: {ProjectId}, Date: {Date}", userId, dto.ProjectId, dto.Date);
            ...
            try
            {
                ...
                var isMember = await _projectRepository.FindAsync(p => p.Id == dto.ProjectId && p.ProjectMembers.Any(m => m.UserId == userId));

                if (!isMember.Any())
                    throw new ForbiddenException("Vous n'êtes pas membre de ce projet.");

                var userRate = await GetActiveUserRateAsync(userId);
                var holidays = await GetActiveHolidaysAsync();
                ...
                var hourEntry = new HourEntry
                {
                    ...
                    SupervisionHours = dto.SupervisionHours,
                    ProcessHours = dto.ProcessHours,
                    ManagementHours = dto.ManagementHours,
                    RAndDHours = dto.RAndDHours,
                    WorkshopHours = dto.WorkshopHours,
                    OtherHours = dto.OtherHours,
                    Notes = dto.Notes,
                    CreatedAt = DateTime.UtcNow
                };

                hourEntry.Calculate(userRate, holidays);

                await _hourEntryRepository.AddAsync(hourEntry);
                await _hourEntryRepository.SaveChangesAsync();

                _logger.LogInformation("CreateAsync terminé — HourEntryId: {HourEntryId}, TotalHours: {TotalHours}, Cost: {TotalCost} {Currency}",
                    hourEntry.Id, hourEntry.TotalHours, hourEntry.TotalCost, hourEntry.Currency);

                return hourEntry.ToDto(project.Name, $"{user.FirstName} {user.LastName}");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Erreur CreateAsync — User: {UserId}, Project: {ProjectId}", userId, dto.ProjectId);
                throw;
            }
        }
        ...
        public async Task<HourEntryDto> UpdateAsync(Guid id, UpdateHourEntryDto dto, Guid userId)
        {
            ...
            try
            {
                ...
                if (entry.UserId != userId)
                    throw new ForbiddenException("Vous ne pouvez modifier que vos propres allocations.");
                ...
                entry.Calculate(userRate, holidays);
        ...
        public async Task DeleteAsync(Guid id, Guid userId)
        {
            ...
            try
            {
                ...
                if (entry.UserId != userId)
                    throw new ForbiddenException("Vous ne pouvez supprimer que vos propres allocations.");
        ...
        public async Task<MonthlyHoursDashboardDto> GetMonthlyDashboardAsync(Guid userId, int year, int month)
        {
            ...
            var workingDays = 22;
            var targetHours = workingDays * 9m;
        ...
        public async Task ApprovePremiumAsync(ApprovePremiumDto dto)
        {
            ...
            try
            {
                ...
                if (!entry.IsPremium)
                    throw new BadRequestException("Cette entrée n'est pas premium.");

                if (entry.PremiumApprovalStatus != Domain.Enums.ApprovalStatus.Pending)
                    throw new BadRequestException("Cette entrée a déjà été traitée.");
        ...
        private async Task<IEnumerable<Holiday>> GetActiveHolidaysAsync()
        {
            _logger.LogInformation("GetActiveHolidaysAsync appelé");
            return await _holidayRepository.FindAsync(h => h.IsActive);
        }
    }
}